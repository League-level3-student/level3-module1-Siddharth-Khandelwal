...
public class MurderOfCrows {
	...
	private void findTheDiamond() {
		...
		for (int i = 0; i < theMurder.size(); i++) {
			ArrayList<String> things =	theMurder.get(i).getStomachContents();
			for (int j = 0; j < things.size(); j++) {
				if(things.get(j).equals("diamond")) {

					System.out.println(""+(theMurder.get(i).getName())+" had the diamond!");
					if(i == 1) {
						System.out.println(i+" innocent crow died to find the diamond.");
					} else {
						System.out.println(i+" innocent crows died to find the diamond.");
					}
